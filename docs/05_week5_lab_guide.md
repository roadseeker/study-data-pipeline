...
Week 4에서 Flink 기반 실시간 스트림 처리 파이프라인이 완성되었다. 5분 단위 윈도우 집계와 이상거래 탐지가 실시간으로 작동하고 있다. 그러나 페이넥스(PayNex) CFO가 새로운 요구사항을 제시한다.
...
| PayNex 활용 | 5분 집계, 이상거래 알림 | 일일 정산, 감사 추적, 품질 검증 |
...
- PayNex 시나리오: 매일 새벽 3시에 전일 거래 전체를 읽어 정산 리포트 생성
- 장점: 전수 데이터 처리로 완전한 정확성 보장
- 단점: 처리 지연(레이턴시)이 존재 — 결과를 즉시 볼 수 없음

## Spark vs Flink — PayNex 관점 정리
...
kafka:
  ...
  source_topic: "paynex.events.ingested"
...
- Week 2~4에서 사용한 PaynexEvent 스키마와 호환
"""
...
def run_bronze_from_file(file_path: str, processing_date: date):
    ...
    bronze_df = (
        ...
        .withColumn("kafka_topic", F.lit("paynex.events.ingested"))
...
def run_full_pipeline(processing_date: date = None):
    ...
    print("=" * 70)
    print(f"[PayNex 배치 ETL] 전체 파이프라인 시작")
    print(f"  처리 일자: {processing_date}")
    ...
    # ── 결과 요약 ──
    print("\n" + "=" * 70)
    print(f"[PayNex 배치 ETL] 전체 파이프라인 완료")
    print(f"  처리 일자: {processing_date}")
...
#!/bin/bash
# PayNex Spark ETL 파이프라인 통합 검증 스크립트
set -e
...
| 소스/대상 | 경로 | 포맷 | 파티셔닝 |
|-----------|------|------|----------|
| Kafka 원본 | paynex.events.ingested | JSON | 파티션 6개 |
...
# PayNex 배치 ETL 아키텍처 — Week 5 완성

## 전체 흐름

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                    PayNex 데이터 파이프라인 — Week 5 완성                       │
│                                                                              │
│  [수집 계층 — Week 2~3]                                                      │
│  ┌──────────┐                                                                │
...
│  [버퍼 계층 — Week 2]                                                        │
│  ┌─────────────────────────────────┐                                         │
│  │ Kafka (paynex.events.ingested)  │  6 파티션, RF=3                         │
│  └──────────┬──────────────────────┘                                         │
│             │                                                                │
│     ┌───────┴──────────┐                                                     │
│     ▼                  ▼                                                     │
