...
> 완료 항목: `config/kafka/topic-naming-convention.md`, `config/kafka/partition-sizing-guide.md`, `scripts/partition-calculator.sh`, `config/kafka/topic-configs.md` 작성 및 `nexuspay.transactions.*` 토픽 생성 확인.
...
> 완료 항목: `scripts/producer_nexuspay.py`, `scripts/verify_partition_key.py` 작성 및 `python scripts/verify_partition_key.py` 실행으로 동일 `user_id`의 단일 파티션 배치 검증 통과 확인.
...
같은 방식으로 `scripts/consumer_fraud_detection.py`, `scripts/consumer_settlement.py`,
`scripts/verify_partition_key.py`의 `BROKER` 값도 동일하게 맞춘다.
...
현재 완료 현황: 12개 항목 중 10개 완료. 미완료 항목은 `docs/fault-tolerance-report.md`, `Git 커밋`이다.
...
| 4 | scripts/partition-calculator.sh (파티션 산정 스크립트) | ☑ |
| 5 | scripts/producer_nexuspay.py (거래 프로듀서) | ☑ |
| 6 | scripts/verify_partition_key.py (파티션 키 검증) | ☑ |
| 7 | scripts/consumer_fraud_detection.py (이상거래 탐지 컨슈머) | ☑ |
| 8 | scripts/consumer_settlement.py (정산 컨슈머) | ☑ |
| 9 | docker-compose.yml 업데이트 (3-브로커 클러스터) | ☑ |
| 10 | docs/fault-tolerance-report.md (장애 복원력 보고서) | ☑ |
| 11 | docs/kafka-operations-guide.md (Kafka 운영 가이드) | ☑ |
| 12 | Git 커밋 | ☐ |
