...
**산출물**: Kafka 운영 가이드 + PayNex 거래 파이프라인 + 장애 복원력 검증 보고서
...
페이넥스(PayNex) PoC 환경 구축이 완료되었다. 이제 CTO가 구체적인 질문을 던진다.
...
컨설턴트로서 이 요구사항을 충족하는 Kafka 토픽 아키텍처를 설계하고, 파티션 전략·컨슈머 그룹 분리·오프셋 관리·장애 복원력까지 검증하는 것이 이번 주의 과제다.
...
### 1-1. PayNex 이벤트 모델 정의

페이넥스의 거래 시스템에서 발생하는 핵심 이벤트를 정의한다.

```
┌─────────────────────────────────────────────────────┐
│                   PayNex 이벤트 흐름                   │
│                                                       │
│  [결제 서비스] ──→ paynex.transactions.payment        │
│  [이체 서비스] ──→ paynex.transactions.transfer       │
│  [출금 서비스] ──→ paynex.transactions.withdrawal     │
│                        │                              │
│                        ▼                              │
│         ┌──────────────┴──────────────┐               │
│         │                             │               │
...
예시:
  paynex.transactions.payment      — 결제 이벤트
  paynex.transactions.transfer     — 이체 이벤트
  paynex.transactions.withdrawal   — 출금 이벤트
  paynex.transactions.dlq          — 처리 실패 메시지 (Dead Letter Queue)
  paynex.users.signup              — 회원가입 이벤트
  paynex.users.profile-update      — 프로필 변경 이벤트
```
...
| 토픽명 | 파티션 | 보존 기간 | 용도 |
|--------|--------|----------|------|
| paynex.transactions.payment | 6 | 7일 | 결제 이벤트 |
| paynex.transactions.transfer | 6 | 7일 | 이체 이벤트 |
| paynex.transactions.withdrawal | 4 | 7일 | 출금 이벤트 |
| paynex.transactions.dlq | 2 | 30일 | 처리 실패 메시지 |
| paynex.users.signup | 2 | 7일 | 회원가입 이벤트 |
...
파티션 수는 목표 처리량과 컨슈머 병렬성에 의해 결정된다. PayNex 시나리오 기준으로 산정 로직을 수립한다.

```bash
cat > scripts/partition-calculator.sh << 'SCRIPT'
#!/bin/bash
# PayNex 파티션 수 산정 스크립트
#
...
docker exec lab-kafka /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:9092 \
  --delete --topic paynex-transactions 2>/dev/null

# 거래 토픽 생성
for TOPIC in paynex.transactions.payment paynex.transactions.transfer; do
  docker exec lab-kafka /opt/kafka/bin/kafka-topics.sh \
    --bootstrap-server localhost:9092 \
    --create --topic "$TOPIC" \
    --partitions 6 \
...
docker exec lab-kafka /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:9092 \
  --create --topic paynex.transactions.withdrawal \
  --partitions 4 \
  --replication-factor 1 \
  --config retention.ms=604800000

# DLQ 전략:
#   - paynex.transactions.dlq : 프로듀서/컨슈머 처리 실패 메시지 (Week 2 프로듀서·컨슈머 단계)
#   - paynex.events.dlq       : NiFi 수집 품질 검증 실패 메시지 (Week 3에서 별도 생성)
#   두 DLQ는 용도가 다르므로 분리 운영한다.
docker exec lab-kafka /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:9092 \
  --create --topic paynex.transactions.dlq \
  --partitions 2 \
  --replication-factor 1 \
...
docker exec lab-kafka /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server localhost:9092 \
  --describe --topic paynex.transactions.payment
```
...
TOPICS = {
    "PAYMENT":    "paynex.transactions.payment",
    "TRANSFER":   "paynex.transactions.transfer",
    "WITHDRAWAL": "paynex.transactions.withdrawal",
}
...
def main():
    parser = argparse.ArgumentParser(description="PayNex 거래 프로듀서")
    ...
    print(f"PayNex 거래 프로듀서 시작 — {args.count}건 전송")
    print(f"브로커: {BROKER}")
    print("=" * 60)
...
```bash
# 거래 100건 생성 (상세 모드)
python scripts/producer_paynex.py -n 100 --delay 0.05 --verbose

# 파티션별 메시지 분포 확인
echo "=== paynex.transactions.payment 파티션별 오프셋 ==="
docker exec lab-kafka /opt/kafka/bin/kafka-run-class.sh \
  kafka.tools.GetOffsetShell \
  --broker-list localhost:9092 \
  --topic paynex.transactions.payment

echo ""
echo "=== paynex.transactions.transfer 파티션별 오프셋 ==="
docker exec lab-kafka /opt/kafka/bin/kafka-run-class.sh \
  kafka.tools.GetOffsetShell \
  --broker-list localhost:9092 \
  --topic paynex.transactions.transfer

echo ""
echo "=== paynex.transactions.withdrawal 파티션별 오프셋 ==="
docker exec lab-kafka /opt/kafka/bin/kafka-run-class.sh \
  kafka.tools.GetOffsetShell \
  --broker-list localhost:9092 \
  --topic paynex.transactions.withdrawal
```
...
PayNex 시나리오에서 두 팀이 **같은 토픽을 독립적으로** 소비해야 한다. Kafka의 컨슈머 그룹이 이를 해결한다.

```
paynex.transactions.payment (6개 파티션)
        │
...
BROKER = "localhost:29092"
TOPICS = [
    "paynex.transactions.payment",
    "paynex.transactions.transfer",
    "paynex.transactions.withdrawal",
]
...
python scripts/producer_paynex.py -n 500 --delay 0.02
```
...
fraud-detection  paynex.transactions.payment      0          42              42              0
fraud-detection  paynex.transactions.payment      1          38              38              0
fraud-detection  paynex.transactions.payment      2          45              45              0
...
```
...
docker exec lab-kafka-1 /opt/kafka/bin/kafka-metadata.sh \
  ...
  --cluster-id PayNexLabCluster2026 2>/dev/null || \
...
기대 출력:
```
Topic: paynex.transactions.payment  PartitionCount: 6  ReplicationFactor: 3
  Topic: paynex.transactions.payment  Partition: 0  Leader: 1  Replicas: 1,2,3  Isr: 1,2,3
  Topic: paynex.transactions.payment  Partition: 1  Leader: 2  Replicas: 2,3,1  Isr: 2,3,1
  Topic: paynex.transactions.payment  Partition: 2  Leader: 3  Replicas: 3,1,2  Isr: 3,1,2
  ...
```
...
```python
# scripts/producer_paynex.py 수정 — BROKER 변경
# 단일 브로커:
# BROKER = "localhost:29092"
...
python scripts/producer_paynex.py -n 300 --delay 0.02
...
python scripts/producer_paynex.py -n 50 --delay 0.02
...
echo ""
echo "=== 프로듀서 전송 시도 ==="
python scripts/producer_paynex.py -n 5 --delay 1
...
docker exec lab-kafka-1 /opt/kafka/bin/kafka-configs.sh \
  --bootstrap-server kafka-1:9092 \
  --entity-type topics \
  --entity-name paynex.transactions.payment \
...
| 4 | scripts/producer_paynex.py (거래 프로듀서) | ☐ |
