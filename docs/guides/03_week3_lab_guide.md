...
- InvokeHTTP → SplitJson → UpdateAttribute → ExecuteScript → JoltTransformJSON → EvaluateJsonPath → Output Port

### PG-2: File Ingestion (정산 CSV 수집)
- ListFile → FetchFile → ConvertRecord(CSV→JSON) → SplitRecord → UpdateAttribute → ExecuteScript → JoltTransformJSON → EvaluateJsonPath → Output Port

### PG-3: DB Ingestion (고객 마스터 수집)
- QueryDatabaseTable → ConvertRecord(Avro→JSON) → SplitRecord → UpdateAttribute → ExecuteScript → JoltTransformJSON → EvaluateJsonPath → Output Port

### PG-4: Schema Standardization (스키마 표준화)
- Input Port(api-in / file-in / db-in) → LogAttribute(*-in-check) → ValidateRecord
  - valid → UpdateAttribute(valid-meta) → Output Port [valid-out]
  - invalid → UpdateAttribute(invalid-meta) → Output Port [invalid-out]
  - failure → LogAttribute [검증실패]

### PG-5: Kafka Publishing (Kafka 전달)
- Input Port(valid-data / invalid-data) → PublishKafka
  - success → LogAttribute(성공 기록)
  - failure → LogAttribute(Kafka 전송 실패)
...
3. `UpdateAttribute`가 `ingested_at`, `source_system`을 추가한다.
4. `ExecuteScript`가 원본 `timestamp`를 UTC 형식으로 정규화한다.
5. `JoltTransformJSON`이 Nexus Pay 표준 이벤트 형태로 바꾼다.
6. `EvaluateJsonPath`가 최종 표준 필드(`event_id`, `event_type`, `event_timestamp`, `data_source`, `source_system`)를 attribute로 만든다.
7. 최종 결과가 `Output Port [api-out]`까지 도달한다.

UI 작업을 시작하기 전에 머릿속에서 아래 한 줄 흐름을 고정해 두면 실습이 훨씬 덜 헷갈린다.

```text
결제 API 호출 -> 응답 수신 -> data 배열 분리 -> 메타데이터 추가 -> timestamp 정규화 -> 표준 JSON 변환 -> 표준 필드 추출 -> api-out 전달
```
...
InvokeHTTP
    ...
    │                                                                │                    │                    │
    │                                                                │                    │                    ├── (matched) → Output Port [api-out]
    │                                                                │                    │                    ├── (unmatched) → LogAttribute [JSON경로실패]
    │                                                                │                    │                    └── (failure) → LogAttribute [JSON경로실패]
    │                                                                │                    └── (failure) → LogAttribute [변환실패]
    │                                                                └── (failure) → LogAttribute [변환실패-TIMESTAMP]
    │
...
3. 이름을 각각 `InvokeHTTP`, `SplitJson`, `UpdateAttribute`, `ExecuteScript`, `JoltTransformJSON`, `EvaluateJsonPath`로 맞춘다.
...
2. `SplitJson`에서 `UpdateAttribute`로 연결하고 relationship은 `split`만 선택한다.
3. `UpdateAttribute`에서 `ExecuteScript`로 연결하고 relationship은 `success`를 선택한다.
4. `ExecuteScript`에서 `JoltTransformJSON`으로 연결하고 relationship은 `success`를 선택한다.
5. `JoltTransformJSON`에서 `EvaluateJsonPath`로 연결하고 relationship은 `success`를 선택한다.
6. `EvaluateJsonPath`에서 `api-out`으로 연결하고 relationship은 `matched`를 선택한다.
7. 실패 라인은 각각 대응하는 `LogAttribute`로 연결한다.
...
연결은 `InvokeHTTP response -> SplitJson`, 그리고 `SplitJson split -> UpdateAttribute`로 설정한다.
...
현재 Week 3의 최종 기준은 `EvaluateJsonPath`를 `JoltTransformJSON` 뒤에 두고, 표준화가 끝난 JSON에서 공통 핵심 필드를 FlowFile Attribute로 추출하는 것이다. 이렇게 하면 `PG-1`, `PG-2`, `PG-3` 모두 같은 필드명으로 PG-4, PG-5에 전달할 수 있다.

`EvaluateJsonPath`의 핵심 역할은 JSON 본문 자체를 바꾸는 것이 아니라, 표준화가 끝난 JSON 안의 공통 필드를 읽어 FlowFile Attribute로 꺼내 놓는 것이다. 이렇게 추출한 attribute는 NiFi가 후속 단계에서 본문을 다시 파싱하지 않고도 라우팅, 로깅, Provenance 검색, Kafka key 지정 작업을 수행할 수 있게 해준다.
...
`ExecuteScript`와 `JoltTransformJSON`에서 공통으로 쓸 메타데이터를 먼저 attribute로 만들어 전달한다.
...
- `SplitJson split -> UpdateAttribute`
...
### 2-8. ExecuteScript — timestamp UTC 정규화

API 원본의 `timestamp`는 `2026-04-14T13:03:10.713212+00:00`처럼 UTC offset을 포함한 문자열이다. 현재 PG-1은 `ExecuteScript`를 Jolt 앞에 두고, 원본 필드인 `timestamp`를 먼저 UTC ISO-8601 형식으로 정규화한 뒤 Jolt에서 `event_timestamp`로 매핑한다.

여기서 목표는 시간대 변환이 아니라, `+00:00` 형태의 UTC offset 표기를 `Z` 형태의 UTC 문자열로 통일하는 것이다. 따라서 이 단계가 끝나면 `timestamp`는 `2026-04-14T13:03:10Z` 형태가 된다.

```bash
cat > config/nifi/scripts/normalize-payment-event-timestamp.groovy << 'EOF'
...
try {
    ...
    flowFile = session.putAttribute(flowFile, "timestamp.normalized", "true")
...
| Script File | `/opt/nifi/custom-config/scripts/normalize-payment-event-timestamp.groovy` |
| Script Body | 비움 |
| Module Directory | 비움 |
...
- `UpdateAttribute success -> ExecuteScript`
- `ExecuteScript success -> JoltTransformJSON`
- `ExecuteScript failure -> LogAttribute [변환실패-TIMESTAMP]`

`LogAttribute [변환실패-TIMESTAMP]` 설정:

| 속성 | 값 |
|------|-----|
| Log Level | `error` |
| Log prefix | `[API-TIMESTAMP-FAIL]` |
| Attributes to Log | `filename, uuid, mime.type, timestamp, source_system, timestamp.normalized` |

### 2-9. JoltTransformJSON 스키마 표준화
...
- 변환 성공 후 `EvaluateJsonPath` 앞 큐에서 최종 JSON 구조를 직접 열어본다

### 2-9A. EvaluateJsonPath — 표준 필드 attribute 추출

`PG-1`, `PG-2`, `PG-3` 모두 `JoltTransformJSON` 뒤에 동일한 `EvaluateJsonPath`를 둔다. 이 단계에서 최종 표준 필드가 attribute로 다시 추출되므로, `PG-4`에서는 `${event_id}`, `${data_source}`, `${source_system}` 같은 공통 표현식을 그대로 재사용할 수 있다.
...
- `JoltTransformJSON success -> EvaluateJsonPath`
- `EvaluateJsonPath matched -> Output Port [api-out]`
- `EvaluateJsonPath unmatched -> LogAttribute [JSON경로실패]`
- `EvaluateJsonPath failure -> LogAttribute [JSON경로실패]`
...
4. `JoltTransformJSON -> EvaluateJsonPath` connection에서 FlowFile 1건을 선택한다.
...
3. `UpdateAttribute` 이후 `ingested_at`, `source_system`이 추가됐는지 확인한다.
4. `ExecuteScript` 이후 `timestamp`가 `2026-04-14T13:03:10Z` 같은 UTC 형식으로 바뀌었는지 확인한다.
5. `JoltTransformJSON` 이후 content가 표준 JSON으로 바뀌었는지 확인한다.
6. `EvaluateJsonPath` 이후 FlowFile Attribute에 `event_id`, `event_type`, `event_timestamp`, `data_source`, `source_system`이 들어갔는지 확인한다.
7. `api-out` 큐에 데이터가 쌓이면 Day 2 범위는 성공이다.
...
- Provenance로 "API 응답 수신 -> JSON 분리 -> 속성 추가 -> timestamp 정규화 -> Jolt 변환 -> 표준 필드 추출" 이력을 따라가며 원인 분석
...
| `EvaluateJsonPath`에서 failure/unmatched 발생 | JsonPath 키 이름 | `event_id`, `event_timestamp`, `data_source` 오타 |
...
**Day 2 완료 기준**: `PG-1: API Ingestion` 내 `InvokeHTTP(response)` → `SplitJson` → `UpdateAttribute` → `ExecuteScript` → `JoltTransformJSON` → `EvaluateJsonPath` 플로우가 정상 작동하고, `api-out` 직전 FlowFile Attribute에 `event_id`, `event_type`, `event_timestamp`, `data_source`, `source_system`가 포함되며, Bulletin Board와 Provenance에서 처리 흐름을 확인하고, `scripts/nifi/measure_api_throughput.sh` 실행 결과까지 점검한 상태.
...
### 3-6B. ExecuteScript — settlement_date UTC 정규화

정산 CSV의 `settlement_date`는 날짜만 제공되므로, 현재 PG-2는 `ExecuteScript`를 Jolt 앞에 두고 원본 필드인 `settlement_date`를 먼저 UTC ISO-8601 형식으로 정규화한 뒤 Jolt에서 `event_timestamp`로 매핑한다.

여기서 목표는 "날짜만 있는 정산 기준일"을 "표준 이벤트 타임스탬프"로 바꾸는 것이다. 따라서 이 단계가 끝나면 `settlement_date`는 더 이상 `2026-04-13` 같은 날짜 문자열이 아니라, 반드시 `2026-04-13T00:00:00Z` 형태의 UTC ISO-8601 문자열이어야 한다.
...
try {
    flowFile = session.write(flowFile, { inputStream, outputStream ->
        ...
        if (json.settlement_date instanceof String &&
            ...
            json.settlement_date = LocalDate.parse(json.settlement_date)
...
| Attributes to Log | `filename, uuid, mime.type, settlement_date, source_system, settlement_date.normalized` |

이 로그 프로세서는 `ExecuteScript`에서 UTC 정규화에 실패한 FlowFile의 핵심 식별값을 `nifi-app.log`에 남긴다. `settlement_date.normalized=true`가 찍히지 않았거나, `settlement_date`가 날짜형 문자열로 남아 있는 경우 이 로그를 통해 즉시 확인할 수 있다.

`PG-2`도 `PG-1`과 동일하게 `JoltTransformJSON` 뒤에 공통 `EvaluateJsonPath`를 둔다. 설정값은 `event_id`, `event_type`, `event_timestamp`, `data_source`, `source_system` 5개를 동일하게 사용하고, `matched -> file-out`, `unmatched/failure -> LogAttribute [JSON경로실패]`로 연결한다.

정상 동작 검증 순서:

1. `UpdateAttribute -> ExecuteScript` 큐에서 FlowFile이 정상적으로 소비되는지 확인한다.
2. `ExecuteScript -> JoltTransformJSON` connection에서 FlowFile 1건을 선택한다.
3. `View data`에서 `settlement_date`가 `2026-04-13T00:00:00Z`처럼 UTC 형식으로 바뀌었는지 확인한다.
4. `Attributes` 탭에서 `settlement_date.normalized = true`가 붙었는지 확인한다.
5. 오류가 발생하면 `ExecuteScript failure -> LogAttribute [변환실패-UTC정규화]`와 `nifi-app.log`를 함께 본다.

완료 판단 기준:

- `JoltTransformJSON` 이후 FlowFile Content에 `event_timestamp`가 반드시 `T00:00:00Z`를 포함한 UTC 문자열로 표시된다.
- 같은 FlowFile Attribute에 `settlement_date.normalized = true`가 기록된다.
- `source_system = nexuspay-settlement-file`, `data_source = settlement-csv`, `ingested_at`가 함께 유지된다.
...
# 2. ListFile이 파일 탐지 → FetchFile이 읽기 → ConvertRecord → SplitRecord → UpdateAttribute → ExecuteScript → Jolt → EvaluateJsonPath
...
### 3-14A. ExecuteScript — DB updated_at UTC 정규화

DB 수집에서는 PostgreSQL `updated_at` 값이 `2026-04-14 05:28:02.068599`처럼 timezone 정보가 없는 문자열로 들어올 수 있다. 현재 PG-3는 `ExecuteScript`를 Jolt 앞에 두고, 원본 필드인 `updated_at`를 먼저 UTC ISO-8601 형식으로 정규화한 뒤 Jolt에서 `event_timestamp`로 매핑한다.
...
try {
    ...
    log.error("DB updated_at UTC 정규화 실패", e)
    session.transfer(flowFile, REL_FAILURE)
}
EOF
```
...
| Attributes to Log | `filename, uuid, mime.type, updated_at, source_system, data_source, event_timestamp.normalized, tablename` |

### 3-15. JoltTransformJSON — 고객 마스터 스키마 표준화

고객 마스터 필드를 표준 스키마에 맞게 변환한다. 앞 단계에서 UTC 형식으로 정규화된 `updated_at`는 여기서 최종 표준 필드 `event_timestamp`로 매핑된다.
...
이 실습에서 `PG-3: DB Ingestion`의 `data_source`는 `customer-db`로 고정한다. 즉 PostgreSQL `customers` 테이블에서 수집한 표준 이벤트는 모두 `data_source = customer-db`, `source_system = nexuspay-customer-db` 기준으로 해석한다.

`PG-3`도 `PG-1`, `PG-2`와 동일하게 `JoltTransformJSON` 뒤에 공통 `EvaluateJsonPath`를 둔다. 설정값은 `event_id`, `event_type`, `event_timestamp`, `data_source`, `source_system` 5개를 동일하게 사용하고, `matched -> db-out`, `unmatched/failure -> LogAttribute [JSON경로실패]`로 연결한다.
...
Input Port [file-in] ─→ LogAttribute [file-in-check] ─┼──→ ValidateRecord
Input Port [db-in]   ─→ LogAttribute [db-in-check]   ─┘        │
                                                                ├── (valid) → UpdateAttribute [valid-meta] → Output Port [valid-out]
                                                                ├── (invalid) → UpdateAttribute [invalid-meta] → Output Port [invalid-out]
                                                                └── (failure) → LogAttribute [검증실패]
```
...
### 4-4. UpdateAttribute — Kafka 메타데이터 추가

`ValidateRecord`의 `valid`, `invalid` 관계는 각각 다른 `UpdateAttribute`로 보낸다. 세 소스 공통 처리 경로에서는 `user_id`가 비어 있을 수 있으므로, Kafka key는 모든 소스에 공통으로 존재하는 `${event_id}`를 사용한다.

**1) UpdateAttribute [valid-meta]**
...
```
Input Port [valid-data] ──→ PublishKafka [ingested-topic]
                                ├── (success) → LogAttribute [전송성공]
                                └── (failure) → LogAttribute [Kafka전송실패]

Input Port [invalid-data] ──→ PublishKafka [dlq-topic]
                                ├── (success) → LogAttribute [DLQ전송]
                                └── (failure) → LogAttribute [Kafka전송실패]
```
...
| 속성 | 값 | 설명 |
|------|-----|------|
| Kafka Connection Service | `Kafka3ConnectionService` | `Bootstrap Servers = kafka-1:9092,kafka-2:9092,kafka-3:9092`, `Security Protocol = PLAINTEXT` |
| Topic Name | `${kafka.topic}` | Attribute에서 동적 결정 |
| Failure Strategy | `Route to Failure` | 전송 실패 시 failure relationship 사용 |
| Delivery Guarantee | `Guarantee Replicated Delivery` | acks=all 동등 |
| Transactions Enabled | `true` | 트랜잭션 전송 활성화 |
| Transactional ID Prefix | `pk-valid` | 정상 경로 전용 prefix |
| Kafka Key | `${kafka.key}` | `event_id` 기반 파티션 키 |
...
| 속성 | 값 | 설명 |
|------|-----|------|
| Kafka Connection Service | `Kafka3ConnectionService` | 위와 동일 |
| Topic Name | `nexuspay.events.dlq` | 고정 DLQ 토픽 |
| Failure Strategy | `Route to Failure` | DLQ 토픽 전송 실패도 별도 로깅 |
| Delivery Guarantee | `Guarantee Replicated Delivery` | |
| Transactions Enabled | `true` | 트랜잭션 전송 활성화 |
| Transactional ID Prefix | `pk-dlq` | DLQ 경로 전용 prefix |
| Kafka Key | `${kafka.key}` | `event_id` 기반 파티션 키 |
| Compression Type | `lz4` | |
