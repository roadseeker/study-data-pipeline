...
`JoltTransformJSON`과 `ExecuteScript`에서 공통으로 쓸 메타데이터를 먼저 attribute로 만들어 전달한다.
...
5. `ExecuteScript` 이후 `timestamp`가 `2026-04-14T13:03:10Z` 같은 UTC 형식으로 바뀌었는지 확인한다.
6. `JoltTransformJSON` 이후 content가 표준 JSON으로 바뀌었는지 확인한다.
...
**Day 2 완료 기준**: `PG-1: API Ingestion` 내 `InvokeHTTP(response)` → `SplitJson` → `EvaluateJsonPath` → `UpdateAttribute` → `ExecuteScript` → `JoltTransformJSON` 플로우가 정상 작동하고, `api-out` 직전 FlowFile에 `ingested_at`, `source_system`, UTC로 정규화된 `event_timestamp`가 포함되며, Bulletin Board와 Provenance에서 처리 흐름을 확인하고, `scripts/nifi/measure_api_throughput.sh` 실행 결과까지 점검한 상태.
...
- **주기**: 30초
- **프로세서 체인**: InvokeHTTP → SplitJson → EvaluateJsonPath → UpdateAttribute → ExecuteScript → JoltTransformJSON
- **처리량**: ~40건/분

### PG-2: File Ingestion
- **소스**: 정산 CSV 파일 (`/data/settlement/settlement_*.csv`)
- **주기**: 파일 감지 시 즉시
- **프로세서 체인**: ListFile → FetchFile → ConvertRecord(CSV→JSON) → SplitRecord → UpdateAttribute → ExecuteScript → JoltTransformJSON
- **처리 완료 파일**: `/data/settlement/processed/`로 이동

### PG-3: DB Ingestion
- **소스**: PostgreSQL `customers` 테이블
- **주기**: 60초 (증분 추출)
- **프로세서 체인**: QueryDatabaseTable → ConvertRecord(Avro→JSON) → SplitRecord → UpdateAttribute → ExecuteScript → JoltTransformJSON
- **증분 기준**: `updated_at` 컬럼
