...
NiFi 웹 UI(`https://localhost:8443/nifi`)에서 다음 프로세서 그룹을 생성한다. 최초 접속 시 self-signed 인증서 경고가 나타나면 예외를 허용한 뒤, `.env`의 `NIFI_USERNAME / NIFI_PASSWORD` 값으로 로그인한다.
...
| Scheduling — Execution | `Primary Node` | 클러스터 환경에서 외부 결제 API polling을 1회만 수행하여 중복 호출 방지 |
| Connection Timeout | `10 sec` | |
| Read Timeout | `30 sec` | |

> **실무 참고**: 프로덕션에서는 API Rate Limiting을 고려하여 Run Schedule을 적절히 설정한다. Nexus Pay 시나리오에서는 30초마다 최근 20건을 가져오는 polling 방식을 사용한다.

> **현재 UI 메모**: `InvokeHTTP`의 `Content-Type`은 기본 프로퍼티로 보이지 않을 수 있다. 이번 GET 호출 시나리오에서는 필수값이 아니므로 별도 설정 없이 진행해도 된다. 필요하면 dynamic property로 `Content-Type=application/json`을 추가할 수 있다.

> **Execution 설정 메모**: `InvokeHTTP`에 self-loop 재시도 connection을 연결하면 incoming connection이 생겨 `Primary Node` 실행과 충돌한다. 따라서 현재 Week 3 시나리오에서는 `retry`, `failure`, `no retry`를 `LogAttribute [API호출실패]`로 보내고, 다음 `Run Schedule` 주기에서 자연 재시도되는 구조를 사용한다. `All Nodes`는 각 NiFi 노드가 동일 API를 각각 호출해야 하는 분산 수집 시나리오에서만 사용한다.
...
> **현재 UI 주의사항**: `Jolt Specification` 안에서 `${now():format(...)}` 같은 동적 시간식을 직접 계산하면 `Invalid Expression Language` 오류가 발생할 수 있다. Week 3 실습에서는 `UpdateAttribute`에서 `ingested_at` attribute를 먼저 만든 뒤, Jolt에서는 `${ingested_at}`만 참조하는 방식으로 구성한다.

### 2-6-1. UpdateAttribute — 수집 시각 attribute 추가

`EvaluateJsonPath` 다음에 `UpdateAttribute`를 하나 배치하고 아래 속성을 설정한다.
...
`config/nifi/jolt-spec-api-payment.json` 파일에 아래 스펙을 저장한다.
...
| Log Level | `warn` |
| Log prefix | `[API-FAIL]` |
| Attributes to Log | `invokehttp.status.code, invokehttp.status.message, invokehttp.request.url, retry.count` |
...
| Attributes to Log | `event_id, event_type, jolt.error, mime.type, error.message, retry.count` |
...
**Day 2 완료 기준**: PG-1 프로세서 그룹 내 InvokeHTTP → SplitJson → EvaluateJsonPath → UpdateAttribute → JoltTransformJSON 플로우 정상 작동, `ingested_at`가 포함된 표준 스키마 FlowFile 확인, 오류 처리 LogAttribute 동작 확인, `scripts/nifi/measure_api_throughput.sh` 실행 결과 확인.
...
### 3-14. JoltTransformJSON — 고객 마스터 스키마 표준화
