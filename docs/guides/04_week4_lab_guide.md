...
public class SessionWindowAnalysisJob {
    ...
    public static class SessionWindowFunction
        ...
        @Override
        public void process(Integer userId, Context ctx, Iterable<SessionAccumulator> accs,
                            Collector<String> out) {
            SessionAccumulator acc = accs.iterator().next();
            out.collect(String.format(
                    "SESSION user=%d, count=%d, total=%.0f, window=[%d, %d)",
                    userId, acc.count, acc.totalAmount,
    ...
    public static void main(String[] args) throws Exception {
        ...
        // AggregateFunction + ProcessWindowFunction 조합으로 건수·총액·윈도우 경계를 모두 수집한다.
        eventStream
