...
- 체크포인트 저장 경로(`file:///opt/flink/checkpoints`)는 `FlinkConfigUtil.configureExactlyOnce()` 내부 `cpConfig.setCheckpointStorage(...)` 값과 반드시 일치시킨다
...
public class FlinkConfigUtil {
    ...
    public static void configureExactlyOnce(StreamExecutionEnvironment env, long checkpointIntervalMs) {
        ...
        // 2. 체크포인트 저장소: 파일 시스템 (JobManager 메모리 기본값은 장애 복구 불가)
        cpConfig.setCheckpointStorage(CHECKPOINT_STORAGE_URI);
     ...
     *
     * Flink 2.x에서 RestartStrategies는 java.time.Duration을 사용한다.
     */
    public static void configureRestartStrategy(StreamExecutionEnvironment env) {
        env.setRestartStrategy(
                RestartStrategies.fixedDelayRestart(3, Duration.ofSeconds(10))
        );
    }
}
JAVAEOF
