...
docs/flink/flink-concepts.md는 Week 4 착수 전에 이미 존재할 수 있다. 기존 내용이 있다면 덮어쓰지 말고 Day 2·Day 4에서 추가로 `cat >>`로 덧붙일 것. 최초 생성이라면 아래 명령을 사용한다.

```bash
# (최초 생성 시) — 기존 파일이 있으면 이 블록을 건너뛰고 내용만 참고한다.
test -e docs/flink/flink-concepts.md || cat > docs/flink/flink-concepts.md << 'EOF'
...
```bash
cat > scripts/flink/flink_event_generator.py << 'PYEOF'
#!/usr/bin/env python3
"""
Flink 실습용 이벤트 생성기.
...
chmod +x scripts/flink/flink_event_generator.py
```
...
cd flink-jobs
# flink-jobs 프로젝트를 Docker 기반으로 빌드하는 명령
...
# Flink 클러스터가 참조할 배포용 JAR 디렉터리에 복사 (Day 1 검증용)
cp target/flink-jobs-1.0.0.jar ../data/flink/usrlib/

# pom.xml 기본 mainClass가 TransactionAggregationJob이지만, 이후 다른 잡도 같은 JAR로 제출하므로
# -c <FullyQualifiedMainClass>를 명시하는 습관을 들인다.
docker exec lab-flink-jm /opt/flink/bin/flink run \
  -d -c com.nexuspay.flink.job.TransactionAggregationJob \
...
python3 scripts/flink/flink_event_generator.py --mode normal --count 20
...
# 기존 RUNNING 잡 모두 취소 + CANCELLED 상태까지 기다림
...
# CANCELLING → CANCELED 전이를 최대 30초까지 대기
for _ in $(seq 1 15); do
  REMAIN=$(curl -s http://localhost:8081/jobs | python3 -c "
import sys, json
print(sum(1 for j in json.load(sys.stdin).get('jobs', []) if j['status'] in ('RUNNING','CANCELLING')))
")
  [ "$REMAIN" = "0" ] && break
  sleep 2
done
...
# 정상 이벤트 생성 (계속 흐르게 두기 위해 백그라운드로 실행)
python3 scripts/flink/flink_event_generator.py --mode normal --count 200 --interval 1 &
GEN_PID=$!

# 1분 슬라이드 간격을 가진 Sliding Window가 먼저 결과를 내므로 이를 먼저 확인
echo "Sliding Window(1분 주기) 결과 대기..."
sleep 75
docker logs lab-flink-tm --tail 50 | grep "SLIDING-5MIN-1MIN"
# 기대: AggResult{type=PAYMENT, window=[...], count=N, total=...}

# 이어서 5분 Tumbling Window 결과 확인
echo "Tumbling Window(5분 주기) 결과 대기..."
sleep 240
docker logs lab-flink-tm --tail 60 | grep "TUMBLING-5MIN"
# 기대: AggResult{type=PAYMENT, window=[...], count=N, total=...}

# 이벤트 생성 종료 후 지연 데이터 테스트
wait $GEN_PID 2>/dev/null || true
python3 scripts/flink/flink_event_generator.py --mode late --count 30

# Late Data Side Output 확인
sleep 30
docker logs lab-flink-tm --tail 30 | grep "LATE-DATA"
...
public class SessionWindowAnalysisJob {
    ...
    public static class SessionAggregateFunction
        ...
        @Override
        public SessionAccumulator add(NexusPayEvent event, SessionAccumulator acc) {
            acc.count++;
            acc.totalAmount += event.getAmount() != null ? event.getAmount() : 0.0;
            return acc;
        }
    ...
    /** 세션 윈도우 메타데이터(start·end·userId)를 결과 문자열에 결합. */
    public static class SessionWindowFunction
        ...
        public void process(Integer userId,
            ...
            out.collect(String.format(
                    "SESSION user=%d, count=%d, total=%.0f, window_start=%s, window_end=%s",
                    userId,
                    acc.count,
                    acc.totalAmount,
...
Day 2에서 Session Window를 유지하는 목적은 고정 길이 윈도우와 다른 "활동 기반 분석" 개념을 익히는 데 있다. Nexus Pay 시나리오에서는 같은 사용자가 짧은 시간에 여러 거래를 발생시키는 구간을 하나의 세션으로 묶고, 2분 이상 비활동이 생기면 다음 세션으로 분리한다. 이 구조는 Day 3 이상 세션 탐지의 전단계 지표를 만드는 기반이 된다.

이번 구현의 학습 포인트는 세 가지다.

1. `EventTimeSessionWindows.withGap(Duration.ofMinutes(2))`로 사용자별 동적 세션 경계를 생성한다.
2. `AggregateFunction + ProcessWindowFunction` 조합으로 건수·총액과 세션 시작/종료 시각을 함께 출력한다.
3. 같은 사용자(`user_id=9001`)가 두 개 세션으로 분리되는 결과를 검증 스크립트로 재현한다.

`SessionWindowAnalysisJob`은 Kafka 소스를 `OffsetsInitializer.latest()` 기준으로 시작한다고 가정한다. 따라서 `verify_session_window.sh`로 동일한 `user_id=9001`, `9002`, `9003` 테스트 데이터를 반복 검증하려면, 검증 직전에 기존 잡을 `CANCEL`하고 새 잡을 다시 실행해야 한다. 잡을 재시작하지 않으면 이전 실행 이후 유입된 이벤트까지 함께 읽을 수 있어 기대 결과가 흔들릴 수 있다.

실행 전에 JAR를 다시 빌드하고, 기존 `RUNNING` 상태의 Session Window 분석 잡을 `CANCEL`한 뒤 새 잡을 제출한다.
...
# 새 잡 제출
docker exec lab-flink-jm /opt/flink/bin/flink run \
  -d -c com.nexuspay.flink.job.SessionWindowAnalysisJob \
  /opt/flink/usrlib/flink-jobs-1.0.0.jar
...
### 2-8. Session Window 동작 검증 스크립트 작성

```bash
cat > scripts/flink/verify_session_window.sh << 'BASHEOF'
#!/usr/bin/env bash
...
chmod +x scripts/flink/verify_session_window.sh
```

### 2-9. Session Window 동작 검증

반복 검증 시에는 아래 순서를 항상 지킨다.

1. 기존 `SessionWindowAnalysisJob`을 `CANCEL`한다.
2. 새 JAR로 잡을 다시 제출한다.
3. Flink REST API에서 잡 상태가 `RUNNING`인지 확인한다.
4. 그 다음 `bash scripts/flink/verify_session_window.sh`를 실행한다.

이 순서를 지켜야 `latest()` 기준으로 새로 유입되는 테스트 이벤트만 읽게 되어, 아래 기대값을 동일하게 재검증할 수 있다.

```bash
bash scripts/flink/verify_session_window.sh
```

위 스크립트는 다음을 한 번에 수행한다.

1. `user_id=9001` 기준으로 두 세션이 분리되도록 고정 이벤트 6건을 전송한다.
2. `user_id=9003`의 미래 시각 이벤트로 워터마크를 전진시켜 앞선 세션이 닫히게 한다.
3. `lab-flink-tm` 로그에서 `SESSION` 출력값을 수집한다.
4. 아래 기대값이 모두 존재하는지 검증한다.

```text
SESSION ... user=9001, count=2, total=200000 ...
SESSION ... user=9001, count=2, total=210000 ...
SESSION ... user=9002, count=1, total=70000 ...
```

기대 결과 해석은 다음과 같다.

- `user=9001` 첫 번째 세션: 35초 이내 2건 거래 → 하나의 세션으로 묶임
- `user=9001` 두 번째 세션: 2분 초과 비활동 후 다시 2건 거래 → 새로운 세션으로 분리됨
- `user=9002` 단건 거래: 별도 단일 세션으로 출력됨

이 검증이 통과하면 Day 2 범위에서 Session Window의 학습 목표, 완성 코드, 동작 검증까지 모두 확보된 상태로 본다.

### 2-10. 윈도우 유형 비교 정리

```bash
cat >> docs/flink/flink-concepts.md << 'EOF'
...
# 이상거래 패턴 이벤트 주입
python3 scripts/flink/flink_event_generator.py --mode fraud --count 50
...
```python
cat > scripts/flink/fraud_alert_redis_sink.py << 'PYEOF'
#!/usr/bin/env python3
"""
...
chmod +x scripts/flink/fraud_alert_redis_sink.py
```

```bash
# Redis 싱크 실행 (백그라운드)
python3 scripts/flink/fraud_alert_redis_sink.py &
...
Day 4에서는 Exactly-once와 TaskManager 장애 복구를 검증해야 하므로, JAR·체크포인트·세이브포인트 저장 경로를 호스트 디렉터리로 바인드 마운트한다. 이 작업을 건너뛰면 `docker stop lab-flink-tm` 후 재기동할 때 체크포인트가 보존되지 않아 복구 검증이 실패한다.

먼저 호스트 디렉터리를 준비한다.

```bash
mkdir -p data/flink/usrlib
mkdir -p data/flink/checkpoints
mkdir -p data/flink/savepoints
```

`docker-compose.yml`의 Flink 서비스 두 개 모두 아래 마운트를 추가한다. **JobManager·TaskManager에 동일하게 적용해야 한다.**

```yaml
  flink-jobmanager:
    volumes:
      - ./data/flink/usrlib:/opt/flink/usrlib
...
적용 시점과 주의사항:
- Day 4 Exactly-once 실습 직전까지 반드시 반영
- Day 1~3에도 동일 구조를 써도 무방하며, 이 경우 빌드 후 `cp target/flink-jobs-1.0.0.jar data/flink/usrlib/`로 배포용 JAR만 갱신한다
- 체크포인트 저장 경로(`file:///opt/flink/checkpoints`)는 `FlinkConfigUtil.configureExactlyOnce()` 내부 `CheckpointingOptions.CHECKPOINTS_DIRECTORY` 값과 반드시 일치시킨다
- savepoint 경로를 분리해 두면 업그레이드·복구 리허설에 유리함
- `data/flink/usrlib`에는 최종 JAR만 두고 `classes`, `maven-status` 같은 Maven 산출물은 넣지 않는다
- 컨테이너 내부에서 쓰기 권한 문제가 나면 `sudo chown -R 9999:9999 data/flink`로 Flink 기본 UID(`flink`=9999)에 맞춘다
...
Flink 2.2에서는 `CheckpointingMode`가 `org.apache.flink.core.execution` 패키지에 있다. 예전 예제에서 보이는 `org.apache.flink.streaming.api.CheckpointingMode` import는 사용하지 않는다.

`TransactionAggregationJob`에서 공통으로 재사용할 상세 체크포인트 설정 유틸리티를 추가한다.
...
public class FlinkConfigUtil {
    ...
    public static void configureExactlyOnce(StreamExecutionEnvironment env, long checkpointIntervalMs) {
        ...
        // 6. 외부화 체크포인트 보존 정책은 Flink 2.2 클러스터 설정에서 관리한다.
        // Day 4 실습에서는 file:///opt/flink/checkpoints 저장소와 장애 복구 동작을 검증한다.

        // 7. 연속 실패 허용: 3회까지 허용 후 잡 실패
        cpConfig.setTolerableCheckpointFailureNumber(3);

        // 8. Unaligned Checkpoint: 백프레셔 상황에서도 체크포인트 진행
...
public class TransactionAggregationJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Tumbling·Sliding 각각 별도 태그를 사용해 두 경로의 late data를 모두 DLQ로 수집한다.
        SingleOutputStreamOperator<AggregatedResult> tumblingResult = txStream
                .keyBy(NexusPayEvent::getEventType)
                .window(TumblingEventTimeWindows.of(Duration.ofMinutes(5)))
                .allowedLateness(Duration.ofSeconds(10))
...
# 브로커의 transaction.max.timeout.ms > Flink 프로듀서 TRANSACTION_TIMEOUT_CONFIG 이어야 한다.
# 현재: 브로커 15분(기본) > Flink 10분 → OK
docker exec lab-kafka-1 /opt/kafka/bin/kafka-configs.sh \
  --bootstrap-server kafka-1:9092 \
  --entity-type brokers \
  --entity-default \
  --describe | grep transaction.max.timeout
# 기본값 900000ms (15분) 확인 — Flink 600000ms(10분)와 여유 5분 확보
```
...
# 기존 잡 모두 취소 후 CANCELLING까지 해소되는지 확인
for JOB_ID in $(curl -s http://localhost:8081/jobs | python3 -c "
import sys, json
for j in json.load(sys.stdin).get('jobs', []):
    if j['status'] == 'RUNNING': print(j['id'])
...
# v3.0 잡 제출 (mainClass 명시 권장)
docker exec lab-flink-jm /opt/flink/bin/flink run \
  -d -c com.nexuspay.flink.job.TransactionAggregationJob \
  /opt/flink/usrlib/flink-jobs-1.0.0.jar

# 잡 ID 확인 — 새로 제출된 RUNNING 잡 ID만 추출
sleep 3
...
# ── 검증 시나리오 A: 정상 흐름 확인 ──
python3 scripts/flink/flink_event_generator.py --mode normal --count 100
...
# 이벤트 주입 시작 (백그라운드)
python3 scripts/flink/flink_event_generator.py --mode normal --count 200 &
GENERATOR_PID=$!
...
```bash
cat > scripts/flink/monitor_checkpoints.sh << 'SHEOF'
#!/bin/bash
# Flink 체크포인트 상태 모니터링
...
chmod +x scripts/flink/monitor_checkpoints.sh
```
...
```bash
cat > scripts/flink/verify_flink_pipeline.sh << 'SHEOF'
#!/bin/bash
...
echo "$FLINK_STATUS" | python -c "import sys,json; d=json.load(sys.stdin); exit(0 if d.get('taskmanagers',0)>0 else 1)" 2>/dev/null
check $? "Flink TaskManager 활성"

SLOTS=$(echo "$FLINK_STATUS" | python -c "import sys,json; print(json.load(sys.stdin).get('slots-available',0))" 2>/dev/null)
echo "     사용 가능 슬롯: $SLOTS"

# 2. Flink 잡 실행 상태
echo ""
echo "[2] Flink 잡 상태"
RUNNING_JOBS=$(curl -sf http://localhost:8081/jobs 2>/dev/null | python -c "
import sys,json
...
echo ""
echo "[4] 이벤트 주입 테스트"
python scripts/flink/flink_event_generator.py --mode fraud --count 30
check $? "이벤트 생성기 실행 성공"
...
echo ""
echo "[5] 체크포인트 상태"
for JOB_ID in $(curl -sf http://localhost:8081/jobs 2>/dev/null | python -c "
import sys,json
for j in json.load(sys.stdin).get('jobs',[]):
    if j['status']=='RUNNING': print(j['id'])
" 2>/dev/null); do
...
chmod +x scripts/flink/verify_flink_pipeline.sh
bash scripts/flink/verify_flink_pipeline.sh
```

### 5-2. Flink 운영 가이드 작성

```bash
cat > docs/flink/flink-operations-guide.md << 'EOF'
...
```bash
cat > docs/flink/flink-architecture.md << 'EOF'
...
2026-04-18 검증 기준으로 Day 1 완료 조건을 충족했다. Flink 핵심 개념 문서 작성, Maven 프로젝트 빌드, Kafka 소스 연동 잡 `RUNNING`, 테스트 이벤트 20건 수신을 확인했다.
2026-04-18 검증 기준으로 Day 2 완료 조건도 충족했다. Tumbling·Sliding·Session 윈도우 집계 결과, Watermark 기반 이벤트 타임 처리, Late Data Side Output 동작을 확인했다.
2026-04-21 검증 기준으로 Day 3 완료 조건을 충족했다. `FraudDetectionJob` 실행 후 `RULE-001`, `RULE-002`, `RULE-003` 알림이 `nexuspay.alerts.fraud` 토픽에 전달되는 것을 확인했고, Redis `fraud:alert:latest:{user_id}`, `fraud:alert:count:{user_id}`, `fraud:alerts:recent` 캐싱 동작을 검증했다.
2026-04-23 검증 기준으로 Day 4 완료 조건을 충족했다. `TransactionAggregationJob v3.0`이 `RUNNING` 상태이고, `nexuspay.aggregation.5min`·`nexuspay.aggregation.sliding` 토픽에 집계 결과가 발행되며, TaskManager 장애 주입 후 체크포인트 `32`에서 복구되고 이후 체크포인트가 계속 완료되는 것을 확인했다.
2026-04-23 검증 기준으로 Day 5 통합 검증 스크립트가 `9건 통과 / 0건 실패`로 완료되었다.

| # | 산출물 | 완료 |
|---|--------|------|
| 1 | docs/flink/flink-concepts.md (Flink 핵심 개념 정리) | ☑ |
...
| 16 | scripts/flink/flink_event_generator.py (테스트 이벤트 생성기) | ☑ |
| 17 | scripts/flink/fraud_alert_redis_sink.py (Redis 알림 캐싱) | ☑ |
| 18 | scripts/flink/verify_flink_pipeline.sh (통합 검증 스크립트) | ☑ |
| 19 | scripts/flink/monitor_checkpoints.sh (체크포인트 모니터링) | ☑ |
| 20 | docs/flink/flink-operations-guide.md (Flink 운영 가이드) | ☑ |
| 21 | docs/flink/flink-architecture.md (파이프라인 아키텍처 문서) | ☑ |
| 22 | Git 커밋 | ☑ |
