...
**전제 조건**: Week 1 환경 정상 기동 (`bash scripts/foundation/healthcheck-all.sh` 전체 통과)
...
산정 근거와 가정의 상세 설명은 [config/kafka/partition-sizing-guide.md](/c:/study/study-data-pipeline/config/kafka/partition-sizing-guide.md)에 정리한다. 이 문서에는 평균 처리량과 피크 처리량 해석, 이상거래 탐지 컨슈머 3대·정산 컨슈머 2대 가정 이유, PoC 환경 축소 적용 기준이 포함된다.

```bash
cat > scripts/kafka/partition-calculator.sh << 'SCRIPT'
#!/bin/bash
...
chmod +x scripts/kafka/partition-calculator.sh
bash scripts/kafka/partition-calculator.sh
```
...
Kafka는 Day 4의 멀티 브로커·복제·장애 복구 실습 전에 영속 볼륨을 붙여 두는 것이 안전하다. 토픽 메타데이터와 브로커 로그를 유지해야 재기동 후에도 동일한 상태를 재현하기 쉽다.
...
적용 시점:
- Day 4 브로커 장애 실습 직전
- 이미 브로커를 띄운 상태라면 `docker compose up -d --force-recreate kafka-1 kafka-2 kafka-3`
- 재생성 전 토픽 목록과 검증 결과는 문서나 메모로 남겨둘 것

### 1-5. 토픽 생성

Week 1에서 만든 단일 브로커 환경에서 토픽을 생성한다 (Day 4에서 멀티 브로커로 확장 시 재생성).

> **Git Bash 주의**: Windows Git Bash에서는 `docker exec <container> /opt/...` 형식이 `/opt/...`를 `C:/Program Files/Git/...`로 잘못 변환해 실패할 수 있다. 이 문서의 Kafka CLI 예시는 모두 `docker exec <container> sh -c '/opt/kafka/bin/...'` 패턴으로 실행한다.
...
# 필요 시 기존 테스트 토픽 정리
for TOPIC in test-transactions nexuspay-transactions; do
  if docker exec lab-kafka sh -c "/opt/kafka/bin/kafka-topics.sh \
    --bootstrap-server localhost:9092 --list | grep -Fx \"$TOPIC\"" > /dev/null; then
    docker exec lab-kafka sh -c "/opt/kafka/bin/kafka-topics.sh \
      --bootstrap-server localhost:9092 \
      --delete --topic \"$TOPIC\""
    echo "Deleted: $TOPIC"
  else
    echo "Skip: $TOPIC (not found)"
  fi
done

> **Kafka WARNING 해설**: `Due to limitations in metric names...` 경고는 Kafka가 점(`.`)과 밑줄(`_`)이 섞인 토픽명을 메트릭에서 구분하기 어려울 수 있음을 알리는 일반 경고다. Nexus Pay 실습은 `nexuspay.transactions.payment`처럼 점(`.`)만 사용하는 규칙을 따르므로 현재 출력은 치명적 오류가 아니며 계속 진행해도 된다. 다만 `nexuspay_transactions_payment` 같은 밑줄 기반 토픽을 함께 만들지는 않는다.
...
> **진행 기록 (2026-04-03)**: Day 1 완료.  
> 완료 항목: `config/kafka/topic-naming-convention.md`, `config/kafka/partition-sizing-guide.md`, `scripts/kafka/partition-calculator.sh`, `config/kafka/topic-configs.md` 작성 및 `nexuspay.transactions.*` 토픽 생성 확인.
...
```python
# scripts/kafka/producer_nexuspay.py
"""
...
def verify_broker_connection(producer, timeout=5):
    """브로커 연결 상태를 시작 시점에 확인한다."""
    try:
        ...
        raise RuntimeError(
            ...
            "`docker compose up -d kafka`로 브로커를 기동하고 "
            "포트 30092 노출 여부를 확인한다."
        ) from exc

    if not metadata.brokers:
        raise RuntimeError(
            "Kafka 메타데이터 조회 결과 브로커가 없습니다. "
            f"bootstrap.servers={BROKER}"
        )

    return ", ".join(f"{broker.host}:{broker.port}" for broker in metadata.brokers.values())


def make_delivery_report(state, verbose):
    """전송 성공·실패 건수를 추적한다."""
    def callback(err, msg):
        if err:
            state["delivery_failed"] += 1
            print(f"  ❌ 전송 실패: topic={msg.topic()} key={msg.key().decode()} error={err}")
            return
...
def flush_or_warn(producer, state, sent, total):
    """flush() 이후 남은 메시지 수를 확인한다."""
    remaining = producer.flush(timeout=10)
    if remaining > 0:
        state["flush_failed"] += remaining
        print(
            "  ❌ flush 타임아웃: "
            f"{remaining}건 미전송 상태 "
            f"(progress={sent}/{total}, broker={BROKER})"
        )
        return False
...
def main():
    ...
    print(f"연결된 브로커: {connected_brokers}")
    ...
    for i in range(1, args.count + 1):
        ...
        if i % 50 == 0 or i == args.count:
            if not flush_or_warn(producer, delivery_state, i, args.count):
                failed = True
                break
    ...
    print(f"  성공 전송: {delivery_state['delivered']}건")
    print(f"  delivery 실패: {delivery_state['delivery_failed']}건")
    print(f"  flush 실패: {delivery_state['flush_failed']}건")
...
```python
# scripts/kafka/verify_partition_key.py
"""
...
```python
# scripts/kafka/consumer_fraud_detection.py
"""
이상거래 탐지 컨슈머
...
```python
# scripts/kafka/consumer_settlement.py
"""
정산 컨슈머
...
```bash
# 등록된 컨슈머 그룹 목록
docker exec lab-kafka sh -c '/opt/kafka/bin/kafka-consumer-groups.sh \
  --bootstrap-server localhost:9092 --list'
...
# → 파티션이 2개 인스턴스로 재분배됨
# 예:
#   ↩ 리밸런싱 시작: 회수=nexuspay.transactions.payment[3], ...
#   ↪ 리밸런싱 완료: 할당=nexuspay.transactions.payment[0], ...
...
> **진행 기록 (2026-04-07)**: Day 3 완료.  
> 완료 확인 항목: `fraud-detection`/`settlement` 그룹 동시 소비 확인, 수동 오프셋 커밋과 lag 해석 확인, `consumer_fraud_detection.py`의 리밸런싱 로그 출력 확인, `fraud-detection` 그룹의 `nexuspay.transactions.payment` 오프셋 리셋 후 재처리 확인.
...
```yaml
  ...
  kafka-1:
    ...
    environment:
      ...
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka-1:9092,EXTERNAL://localhost:30092
  ...
  kafka-2:
    ...
    environment:
      KAFKA_NODE_ID: 2
      KAFKA_PROCESS_ROLES: broker,controller
      KAFKA_LISTENERS: PLAINTEXT://0.0.0.0:9092,CONTROLLER://0.0.0.0:9093,EXTERNAL://0.0.0.0:30093
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka-2:9092,EXTERNAL://localhost:30093
      KAFKA_LISTENER_SECURITY_PROTOCOL_MAP: PLAINTEXT:PLAINTEXT,CONTROLLER:PLAINTEXT,EXTERNAL:PLAINTEXT
      KAFKA_CONTROLLER_QUORUM_VOTERS: 1@kafka-1:9093,2@kafka-2:9093,3@kafka-3:9093
      KAFKA_CONTROLLER_LISTENER_NAMES: CONTROLLER
      KAFKA_INTER_BROKER_LISTENER_NAME: PLAINTEXT
  ...
  kafka-3:
    ...
    environment:
      KAFKA_NODE_ID: 3
      KAFKA_PROCESS_ROLES: broker,controller
      KAFKA_LISTENERS: PLAINTEXT://0.0.0.0:9092,CONTROLLER://0.0.0.0:9093,EXTERNAL://0.0.0.0:30094
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka-3:9092,EXTERNAL://localhost:30094
      KAFKA_LISTENER_SECURITY_PROTOCOL_MAP: PLAINTEXT:PLAINTEXT,CONTROLLER:PLAINTEXT,EXTERNAL:PLAINTEXT
      KAFKA_CONTROLLER_QUORUM_VOTERS: 1@kafka-1:9093,2@kafka-2:9093,3@kafka-3:9093
      KAFKA_CONTROLLER_LISTENER_NAMES: CONTROLLER
      KAFKA_INTER_BROKER_LISTENER_NAME: PLAINTEXT
...
docker exec lab-kafka-1 sh -c '/opt/kafka/bin/kafka-metadata.sh \
...
docker exec lab-kafka-1 sh -c '/opt/kafka/bin/kafka-broker-api-versions.sh \
...
for i in 1 2 3; do
  echo -n "kafka-$i: "
  docker exec lab-kafka-$i sh -c '/opt/kafka/bin/kafka-topics.sh \
    --bootstrap-server localhost:9092 --list' > /dev/null 2>&1 \
    && echo "✅ 정상" || echo "❌ 실패"
done
```

> **출력 해석 팁**: `kafka-broker-api-versions.sh --bootstrap-server kafka-1:9092` 는 접속 시작점을 `kafka-1`로 주더라도, 메타데이터 응답 과정에서 `kafka-2:9092 (id: 2 ...)` 또는 `kafka-3:9092 (id: 3 ...)`가 먼저 보일 수 있다. 이는 `advertised.listeners` 기준으로 다른 브로커 정보를 반환한 정상 동작이며, 이 한 줄만으로 장애를 의미하지 않는다. 실제로 3개 브로커가 모두 살아 있는지는 바로 아래 `for i in 1 2 3` 검증 루프 결과를 기준으로 판단한다.
...
```python
# scripts/kafka/producer_nexuspay.py
# Week 2 멀티 브로커 클러스터의 외부 접속 포트를 모두 나열한다.
# 첫 번째 브로커가 일시적으로 응답하지 않아도 다른 브로커로 메타데이터 조회를 이어갈 수 있다.
BROKER = "localhost:30092,localhost:30093,localhost:30094"
```

같은 방식으로 `scripts/kafka/consumer_fraud_detection.py`, `scripts/kafka/consumer_settlement.py`,
`scripts/kafka/verify_partition_key.py`의 `BROKER` 값도 동일하게 맞춘다.
...
> **출력 해석 팁**: `awk '{print $8}'`로 집계하면 `Replicas:` 컬럼(`1,2,3` 등)을 세게 되어 리더 분산 검증이 왜곡된다. 이 단계에서는 `Leader:` 라벨이 포함된 줄만 필터링한 뒤, 리더 브로커 ID가 위치한 `$6` 필드를 집계한다.

**Day 4 완료 기준**: 3-브로커 클러스터 정상 기동, 복제 팩터 3 토픽 생성, ISR 3/3 확인, 멀티 브로커 프로듀서 전송 성공.
> 검증 메모: `connected_brokers=localhost:30092, localhost:30093, localhost:30094`, `delivered=300`, `delivery_failed=0`, `flush_failed=0`, 리더 분산 결과 `2 1 / 2 2 / 2 3` 확인.
...
docker exec lab-kafka-1 sh -c '/opt/kafka/bin/kafka-configs.sh \
  --bootstrap-server kafka-1:9092 \
  --entity-type topics \
...
이번 실습에서 확인한 실제 결과를 바탕으로 장애 복원력 보고서를 작성한다. 핵심은 `1대 장애 시 서비스 지속`, `ISR 부족 시 쓰기 차단`, `복구 후 ISR 회복과 leader 재분산 관찰`을 정리하는 것이다.

```bash
cat > docs/kafka/fault-tolerance-report.md << 'EOF'
...
```bash
cat > docs/kafka/kafka-operations-guide.md << 'EOF'
...
현재 완료 현황: 12개 항목 중 12개 완료.

| # | 산출물 | 완료 |
|---|--------|------|
| 1 | config/kafka/topic-naming-convention.md (토픽 명명 규칙) | ☑ |
| 2 | config/kafka/partition-sizing-guide.md (파티션/파티션 키/산정 기준) | ☑ |
| 3 | config/kafka/topic-configs.md (토픽 설정 가이드) | ☑ |
| 4 | scripts/kafka/partition-calculator.sh (파티션 산정 스크립트) | ☑ |
| 5 | scripts/kafka/producer_nexuspay.py (거래 프로듀서) | ☑ |
| 6 | scripts/kafka/verify_partition_key.py (파티션 키 검증) | ☑ |
| 7 | scripts/kafka/consumer_fraud_detection.py (이상거래 탐지 컨슈머) | ☑ |
| 8 | scripts/kafka/consumer_settlement.py (정산 컨슈머) | ☑ |
| 9 | docker-compose.yml 업데이트 (3-브로커 클러스터) | ☑ |
| 10 | docs/kafka/fault-tolerance-report.md (장애 복원력 보고서) | ☑ |
| 11 | docs/kafka/kafka-operations-guide.md (Kafka 운영 가이드) | ☑ |
| 12 | Git 커밋 | ☑ |
