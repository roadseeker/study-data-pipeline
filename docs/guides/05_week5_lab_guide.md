...
Week 5부터는 Delta Lake 산출물과 Spark 체크포인트가 핵심 자산이 되므로 Day 1 셋업 시점에 Spark가 읽고 쓸 호스트 경로를 분명히 준비한다. 현재 랩에서는 전체 `./data`를 통째로 마운트하지 않고, Spark가 실제로 사용하는 입력·레이크하우스·로그 경로만 분리해서 bind mount한다.
...
mkdir -p data/lakehouse/delta/{bronze,silver,gold,checkpoints,quality-reports}
mkdir -p logs/spark
```

Week 5 기준 데이터 디렉터리 표준 구조는 다음과 같다.
...
`docker-compose.yml`의 Spark 서비스에는 아래 마운트를 유지하거나 추가한다. `data/flink`, `data/nifi` 같은 다른 주차 런타임 산출물은 Spark 컨테이너에 노출하지 않는다.
...
적용 시점:
- Day 1 프로젝트 셋업 전에 반영
- `./data/lakehouse` bind mount가 있으면 Delta Lake 산출물은 재기동 후에도 유지
- `/data/sample`, `/data/settlement`는 입력 데이터 경로이므로 읽기 전용으로 마운트한다
- `/data/lakehouse/delta/bronze`, `/data/lakehouse/delta/silver`, `/data/lakehouse/delta/gold`를 Week 5 Medallion 표준 경로로 사용한다
...
│  └────────────────────────────────────────────────┘                  │
│                                                                      │
│  /data/lakehouse/delta/bronze/transactions/                          │
