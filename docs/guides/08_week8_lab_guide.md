...
**전제 조건**: Week 1~7 환경 정상 기동 (`bash scripts/foundation/healthcheck-all.sh` 전체 통과), Week 7 Airflow DAG 정상 실행, Delta Lake Bronze/Silver/Gold 및 CDC 파이프라인 동작 확인
...
python3 scripts/nifi/csv_settlement_generator.py -n 2 -r 40 -i 5

echo "[3/5] Kafka 거래 이벤트 생성"
python3 scripts/kafka/producer_nexuspay.py -n 200 --delay 0.02
