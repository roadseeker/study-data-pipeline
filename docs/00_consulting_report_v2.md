...
#### 1주차
>수행 시나리오: 페이넥스(PayNex)의 데이터 파이프라인 현대화 PoC를 위해 Kafka·NiFi·Flink·Spark·Airflow·Redis·PostgreSQL 전체 스택이 로컬에서 정상 동작하는 실습 환경을 구성합니다. 이후 8주 실습의 기반이 되는 공통 환경을 검증하는 것이 목표입니다.
5일 일정 구성:
...
* `scripts/init-db.sql`: PostgreSQL 초기 스키마와 샘플 데이터 적재
* `scripts/healthcheck-all.sh` + `dags/healthcheck_dag.py`: 통합 헬스체크와 Airflow 기반 환경 검증 자동화
* `README.md`: 로컬 기동 절차, 포트, 장애 대응 기본 가이드 정리

#### 2주차
>수행 시나리오: 페이넥스 CTO가 일 평균 50만 건, 향후 200만 건까지 증가할 거래 이벤트를 Kafka로 수집하되 이상거래 탐지 팀과 정산 팀이 서로 간섭 없이 같은 데이터를 소비할 수 있도록 설계하라고 요구합니다. Kafka 토픽 아키텍처와 장애 복원력을 검증하는 것이 이번 주 과제입니다.
5일 일정 구성:
...
* `scripts/partition-calculator.sh` + `scripts/producer_paynex.py`: 파티션 수 산정과 거래 이벤트 프로듀서 구현
* `scripts/verify_partition_key.py` + `scripts/consumer_fraud_detection.py` + `scripts/consumer_settlement.py`: 파티션 키 검증과 다중 컨슈머 그룹 실습
* `docker-compose.yml` 업데이트 + `docs/fault-tolerance-report.md` + `docs/kafka-operations-guide.md`: 3-브로커 클러스터와 장애 복원력 운영 문서 정리

#### 3주차
>수행 시나리오: 페이넥스 CTO가 결제 API JSON, 레거시 정산 CSV, 고객 마스터 DB 데이터를 하나의 파이프라인으로 통합 수집하고, 감사 대응을 위해 데이터 출처와 흐름이 추적 가능해야 한다고 요구합니다. NiFi 기반 다중 소스 수집과 Provenance 추적 체계를 구축합니다.
5일 일정 구성:
...
* `scripts/api_payment_simulator.py` + `scripts/csv_settlement_generator.py` + `scripts/init-customers.sql`: API·파일·DB 3개 소스 데이터 생성 및 초기화
* `config/nifi/process-group-design.md` + `config/nifi/jolt-spec-*.json`: NiFi 프로세서 그룹 설계와 소스별 표준화 변환 규칙
* `config/nifi/paynex-standard-schema.avsc` + `scripts/verify_nifi_pipeline.sh`: 공통 이벤트 스키마와 종합 검증 스크립트
* `docs/provenance-audit-guide.md` + `docs/nifi-monitoring-guide.md` + `docs/nifi-architecture.md`: 계보 추적, 운영, 아키텍처 문서화

#### 4주차
>수행 시나리오: NiFi를 통해 `paynex.events.ingested` 토픽으로 실시간 이벤트가 유입되자, 페이넥스 CTO가 5분 단위 집계, 실시간 이상거래 탐지, Exactly-once 보장을 요구합니다. Apache Flink로 실시간 변환 계층의 핵심 구간을 완성합니다.
5일 일정 구성:
...
* `flink-jobs/src/main/java/com/paynex/flink/job/TransactionAggregationJob.java` + `FraudDetectionJob.java`: 윈도우 집계와 이상거래 탐지 핵심 잡 구현
...
#### 5주차 : 
>수행 시나리오: 페이넥스(PayNex) CFO가 일별·월별 정산 리포트, 감사 추적(타임 트래블), 데이터 품질 검증을 요구하는 상황에서 Apache Spark 배치 ETL로 대응합니다.
5일 일정 구성:
...
#### 6주차
>수행 시나리오: 페이넥스 CIO가 레거시 MySQL 정산 시스템의 3억 건 데이터를 Delta Lake로 이관해야 하는 과제를 제시합니다. 서비스 중단 없이 이관해야 하며, 테이블 특성에 따라 다른 전략이 필요합니다.
5일 일정 구성:
...
#### 7주차
>수행 시나리오: 페이넥스 COO가 Week 1~6에서 구축한 배치 이관(Spark JDBC), 배치 ETL(Spark), CDC(Debezium) 파이프라인을 Apache Airflow로 운영 자동화하라고 요구합니다. 매일 03:00 이관, 06:00 이전 정산 리포트 완료, CDC 장애·SLA 지연 즉시 알림, 특정 날짜 백필·재처리가 가능해야 합니다.
5일 일정 구성:
...
* `dags/paynex_daily_migration_dag.py` + `spark-jobs/orchestration/master_refresh.py`: Spark JDBC 이관과 마스터 리프레시 DAG
* `dags/paynex_daily_etl_dag.py` + `spark-etl/jobs/publish_gold_report.py` + `spark-etl/jobs/verify_gold_outputs.py`: Week 5 배치 ETL 연계와 Gold 결과 검증 자동화
* `dags/paynex_cdc_monitoring_dag.py` + `dags/paynex_backfill_recovery_dag.py` + `dags/paynex_daily_master_dag.py` + `docs/airflow-operations-guide.md`: CDC 상태 점검, 백필·복구, 단일 스케줄 기반 통합 마스터 DAG, 운영 Runbook

#### 8주차
>수행 시나리오: 페이넥스 CTO와 COO가 Week 1~7에서 구축한 Kafka·NiFi·Flink·Spark·Spark JDBC·Debezium·Airflow 자산을 하나의 운영 시나리오로 끝까지 연결해 고객 수용 테스트 수준으로 검증하라고 요구합니다. API JSON, 정산 CSV, PostgreSQL, MySQL 변경 데이터가 동시에 유입되더라도 실시간 탐지와 배치 리포트가 일관되게 생성되어야 하며, 장애 주입·복구·백필까지 재현 가능해야 합니다.
5일 일정 구성:
...
* `dags/paynex_acceptance_rehearsal_dag.py` + `scripts/e2e/verify_end_to_end.sh` + `spark-etl/jobs/build_pipeline_kpis.py`: 통합 수용 테스트 DAG, End-to-End 검증, 최종 KPI 요약
* `scripts/e2e/run_failure_drills.sh` + `scripts/e2e/collect_pipeline_snapshot.sh`: 장애 주입·복구 리허설과 운영 상태 스냅샷 수집
* `docs/final/failure-drill-report.md` + `docs/final/acceptance-test-report.md` + `docs/final/paynex-final-architecture.md` + `docs/final/portfolio-demo-script.md`: 최종 복구 보고서, 수용 테스트 결과, 아키텍처 문서, 포트폴리오 데모 자료
...
│   │   └── paynex-standard-schema.avsc         (W3) 14필드 통합 Avro 스키마
...
│   ├── paynex_daily_migration_dag.py           (W7) Spark JDBC 배치 이관 DAG
│   ├── paynex_daily_etl_dag.py                 (W7) Spark ETL (Bronze→Silver→Gold) DAG
│   ├── paynex_cdc_monitoring_dag.py            (W7) Kafka Connect CDC 상태 감시 DAG
│   ├── paynex_backfill_recovery_dag.py         (W7) 특정 날짜 백필·재처리 DAG
│   ├── paynex_daily_master_dag.py              (W7) 일일 통합 오케스트레이션 마스터 DAG
│   └── paynex_acceptance_rehearsal_dag.py      (W8) 고객 수용 테스트 리허설 DAG
│
├── plugins/
...
│       ├── paynex-final-architecture.md        (W8) 최종 전체 아키텍처 문서
...
│   ├── producer_paynex.py                      (W2) Kafka 거래 이벤트 프로듀서 (confluent-kafka)
...
│       ├── model/
│       │   ├── PaynexEvent.java                (W4) 14필드 이벤트 POJO
│       │   ├── AggregatedResult.java           (W4) 윈도우 집계 결과 모델
│       │   └── FraudAlert.java                 (W4) 이상거래 알림 모델
│       └── util/
│           ├── PaynexEventDeserializer.java    (W4) JSON→POJO 역직렬화 (SNAKE_CASE 매핑)
