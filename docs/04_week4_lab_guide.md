...
**전제 조건**: Week 1·2·3 환경 정상 기동 (`bash scripts/healthcheck-all.sh` 전체 통과), NiFi → Kafka(`paynex.events.ingested`) 데이터 흐름 정상
...
NiFi 기반 다중 소스 수집 체계 구축이 완료되었다. 결제 API·정산 CSV·고객 DB 세 가지 소스의 데이터가 `paynex.events.ingested` 토픽으로 실시간 유입되고 있다. 페이넥스(PayNex) CTO가 다음 단계 요구사항을 제시한다.
...
2. Kafka 소스 커넥터를 통한 `paynex.events.ingested` 토픽 실시간 소비
...
| PayNex 활용 | Week 5 (Spark 배치 ETL) | Week 4 (실시간 분석) |
...
```bash
mkdir -p flink-jobs/{src/main/java/com/paynex/flink/{job,function,model,util},src/main/resources,src/test/java/com/paynex/flink,lib}
```
...
│   │   │   │   └── FraudAlert.java
│   │   │   └── util/                    # 유틸리티
│   │   │       ├── PaynexEventDeserializer.java
│   │   │       └── FlinkConfigUtil.java
│   │   └── resources/
│   │       └── flink-conf.yaml
│   └── test/java/com/paynex/flink/
                        ...
                        <configuration>
                            ...
                            <transformers>
                                <transformer implementation="org.apache.maven.plugins.shade.resource.ManifestResourceTransformer">
                                    <mainClass>com.paynex.flink.job.TransactionAggregationJob</mainClass>
                                </transformer>
                            </transformers>
                        </configuration>
                    </execution>
...
### 1-4. 데이터 모델 — PaynexEvent POJO

Week 3에서 NiFi가 `paynex.events.ingested` 토픽에 전송하는 표준 스키마에 맞춘 Java POJO를 정의한다.

```bash
cat > flink-jobs/src/main/java/com/paynex/flink/model/PaynexEvent.java << 'JAVAEOF'
package com.paynex.flink.model;

import java.io.Serializable;

/**
 * PayNex 표준 이벤트 모델.
 * Week 3 NiFi 스키마 표준화에서 정의한 paynex-standard-schema.avsc와 1:1 대응.
 *
 * 필드 매핑:
 *   event_id       — 이벤트 고유 ID
 ...
 */
public class PaynexEvent implements Serializable {
    ...
    public String toString() {
        return String.format("PaynexEvent{id=%s, type=%s, user=%d, amount=%.0f, time=%s}",
...
### 1-5. JSON 역직렬화기 — Kafka 메시지 → PaynexEvent

```bash
cat > flink-jobs/src/main/java/com/paynex/flink/util/PaynexEventDeserializer.java << 'JAVAEOF'
package com.paynex.flink.util;

import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.databind.PropertyNamingStrategies;
import com.paynex.flink.model.PaynexEvent;
...
/**
 * Kafka에서 수신한 JSON 바이트를 PaynexEvent POJO로 변환.
 * NiFi 표준 스키마의 snake_case 필드명을 Java camelCase에 매핑.
 */
public class PaynexEventDeserializer implements DeserializationSchema<PaynexEvent> {
    private static final long serialVersionUID = 1L;
    ...
    @Override
    public PaynexEvent deserialize(byte[] message) throws IOException {
        try {
            return getMapper().readValue(message, PaynexEvent.class);
        } catch (Exception e) {
    ...
    @Override
    public boolean isEndOfStream(PaynexEvent event) {
        return false;  // 무한 스트림
    }

    @Override
    public TypeInformation<PaynexEvent> getProducedType() {
        return TypeInformation.of(PaynexEvent.class);
    }
}
JAVAEOF
```
...
Flink가 `paynex.events.ingested` 토픽에서 이벤트를 소비하는 기본 잡을 구성한다.

```bash
cat > flink-jobs/src/main/java/com/paynex/flink/job/TransactionAggregationJob.java << 'JAVAEOF'
package com.paynex.flink.job;

import com.paynex.flink.model.PaynexEvent;
import com.paynex.flink.util.PaynexEventDeserializer;
...
public class TransactionAggregationJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 2. Kafka 소스 구성
        KafkaSource<PaynexEvent> kafkaSource = KafkaSource.<PaynexEvent>builder()
                .setBootstrapServers("kafka-1:9092,kafka-2:9092,kafka-3:9092")
                .setTopics("paynex.events.ingested")
                .setGroupId("flink-aggregation")
                .setStartingOffsets(OffsetsInitializer.latest())
                .setValueOnlyDeserializer(new PaynexEventDeserializer())
                .build();

        // 3. Watermark 전략 — 최대 5초 지연 허용
        WatermarkStrategy<PaynexEvent> watermarkStrategy = WatermarkStrategy
                .<PaynexEvent>forBoundedOutOfOrderness(Duration.ofSeconds(5))
                .withTimestampAssigner((event, timestamp) -> event.getEventTimeMillis())
                .withIdleness(Duration.ofMinutes(1));  // 1분간 데이터 없으면 idle 처리

        // 4. 데이터 스트림 생성
        DataStream<PaynexEvent> eventStream = env
                .fromSource(kafkaSource, watermarkStrategy, "PayNex Kafka Source")
                .filter(event -> event != null)  // 역직렬화 실패 이벤트 제거
                .name("Filter Null Events");

        // 5. 거래 이벤트만 필터링 (CUSTOMER_UPDATE 제외)
        DataStream<PaynexEvent> transactionStream = eventStream
                .filter(event -> {
        ...
        // 7. 잡 실행
        env.execute("PayNex Transaction Aggregation Job v1.0");
    }
}
JAVAEOF
```
...
"""
Flink 실습용 이벤트 생성기.
paynex.events.ingested 토픽에 테스트 이벤트를 주입한다.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Flink 실습용 PayNex 이벤트 생성기")
...
# 기대: PaynexEvent{id=..., type=PAYMENT, user=42, amount=1500000, time=...}
```
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/model/AggregatedResult.java << 'JAVAEOF'
package com.paynex.flink.model;
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/function/TransactionAggregateFunction.java << 'JAVAEOF'
package com.paynex.flink.function;

import com.paynex.flink.model.AggregatedResult;
import com.paynex.flink.model.PaynexEvent;
...
public class TransactionAggregateFunction
        implements AggregateFunction<PaynexEvent, TransactionAggregateFunction.Accumulator, AggregatedResult> {
    ...
    @Override
    public Accumulator add(PaynexEvent event, Accumulator acc) {
        acc.count++;
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/function/TransactionWindowFunction.java << 'JAVAEOF'
package com.paynex.flink.function;
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/function/LateDataSideOutputFunction.java << 'JAVAEOF'
package com.paynex.flink.function;
...
public class LateDataSideOutputFunction {
    ...
    public static final OutputTag<PaynexEvent> LATE_DATA_TAG =
            new OutputTag<PaynexEvent>("late-data") {};
}
JAVAEOF
```
...
import com.paynex.flink.function.LateDataSideOutputFunction;
import com.paynex.flink.function.TransactionAggregateFunction;
import com.paynex.flink.function.TransactionWindowFunction;
import com.paynex.flink.model.AggregatedResult;
import com.paynex.flink.model.PaynexEvent;
import com.paynex.flink.util.PaynexEventDeserializer;
import org.apache.flink.api.common.eventtime.WatermarkStrategy;
...
/**
 * PayNex 거래 실시간 집계 잡 — v2.0 (윈도우 집계 포함).
 *
 * 파이프라인 구조:
 *   Kafka(paynex.events.ingested)
...
public class TransactionAggregationJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ── 5. 거래 이벤트 필터 ──
        DataStream<PaynexEvent> txStream = eventStream
                .filter(event -> {
                    String type = event.getEventType();
                    return "PAYMENT".equals(type) || "TRANSFER".equals(type)
                            || "WITHDRAWAL".equals(type) || "SETTLEMENT".equals(type);
        ...
        SingleOutputStreamOperator<AggregatedResult> tumblingResult = txStream
                .keyBy(PaynexEvent::getEventType)
        ...
        // ── 8. Late Data 처리 ──
        DataStream<PaynexEvent> lateData = tumblingResult
        ...
        // ── 9. 잡 실행 ──
        env.execute("PayNex Transaction Aggregation Job v2.0");
    }
}
JAVAEOF
```
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/job/SessionWindowAnalysisJob.java << 'JAVAEOF'
package com.paynex.flink.job;
...
public class SessionWindowAnalysisJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        WatermarkStrategy<PaynexEvent> wmStrategy = WatermarkStrategy
                .<PaynexEvent>forBoundedOutOfOrderness(Duration.ofSeconds(5))
                .withTimestampAssigner((event, ts) -> event.getEventTimeMillis())
                .withIdleness(Duration.ofMinutes(1));

        DataStream<PaynexEvent> eventStream = env
                .fromSource(kafkaSource, wmStrategy, "PayNex Kafka Source")
        ...
        eventStream
                ...
                .reduce((e1, e2) -> {
                    ...
                    PaynexEvent merged = new PaynexEvent();
        ...
        env.execute("PayNex Session Window Analysis");
    }
}
JAVAEOF
```
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/model/FraudAlert.java << 'JAVAEOF'
package com.paynex.flink.model;
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/function/FraudDetectionFunction.java << 'JAVAEOF'
package com.paynex.flink.function;

import com.paynex.flink.model.FraudAlert;
import com.paynex.flink.model.PaynexEvent;
...
public class FraudDetectionFunction
        extends KeyedProcessFunction<Integer, PaynexEvent, FraudAlert> {
    ...
    // Keyed State
    private transient ListState<PaynexEvent> recentEventsState;
    private transient ValueState<Long> lastCleanupState;

    @Override
    public void open(Configuration parameters) {
        recentEventsState = getRuntimeContext().getListState(
                new ListStateDescriptor<>("recent-events", TypeInformation.of(PaynexEvent.class)));
        lastCleanupState = getRuntimeContext().getState(
                new ValueStateDescriptor<>("last-cleanup", Long.class));
    }

    @Override
    public void processElement(PaynexEvent event, Context ctx, Collector<FraudAlert> out)
            throws Exception {
        ...
        // ── 만료 이벤트 정리 (1분 경과) ──
        List<PaynexEvent> activeEvents = new ArrayList<>();
        Iterator<PaynexEvent> iter = recentEventsState.get().iterator();
        while (iter.hasNext()) {
            PaynexEvent e = iter.next();
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/job/FraudDetectionJob.java << 'JAVAEOF'
package com.paynex.flink.job;

import com.paynex.flink.function.FraudDetectionFunction;
import com.paynex.flink.model.FraudAlert;
import com.paynex.flink.model.PaynexEvent;
import com.paynex.flink.util.PaynexEventDeserializer;
import org.apache.flink.api.common.eventtime.WatermarkStrategy;
 ...
 *     → 알림 싱크: Kafka(paynex.alerts.fraud) + 콘솔
 *
 * 탐지 규칙:
...
public class FraudDetectionJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ── 6. 실행 ──
        env.execute("PayNex Fraud Detection Job v1.0");
    }
}
JAVAEOF
```
...
docker exec lab-kafka-1 /opt/kafka/bin/kafka-topics.sh \
  ...
  --create --topic paynex.alerts.fraud \
  --partitions 3 \
...
docker cp target/flink-jobs-1.0.0.jar lab-flink-jm:/opt/flink/usrlib/
docker exec lab-flink-jm /opt/flink/bin/flink run \
  -d -c com.paynex.flink.job.FraudDetectionJob \
...
#!/usr/bin/env python3
"""
Kafka paynex.alerts.fraud 토픽을 소비하여 Redis에 캐싱.
...
# Redis에서 알림 조회
docker exec lab-redis redis-cli -a redis1234 ZRANGE fraud:alerts:recent 0 -1
docker exec lab-redis redis-cli -a redis1234 HGETALL fraud:alert:latest:42
docker exec lab-redis redis-cli -a redis1234 GET fraud:alert:count:42
```
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/util/FlinkConfigUtil.java << 'JAVAEOF'
package com.paynex.flink.util;
...
```bash
cat > flink-jobs/src/main/java/com/paynex/flink/util/ExactlyOnceKafkaSinkBuilder.java << 'JAVAEOF'
package com.paynex.flink.util;
...
import com.paynex.flink.model.AggregatedResult;
import com.paynex.flink.model.PaynexEvent;
import com.paynex.flink.util.ExactlyOnceKafkaSinkBuilder;
import com.paynex.flink.util.FlinkConfigUtil;
import com.paynex.flink.util.PaynexEventDeserializer;
import org.apache.flink.api.common.eventtime.WatermarkStrategy;
import org.apache.flink.connector.kafka.sink.KafkaSink;
import org.apache.flink.connector.kafka.source.KafkaSource;
...
/**
 * PayNex 거래 실시간 집계 잡 — v3.0 (Exactly-once 적용 최종판).
 *
 * End-to-End 보장:
...
public class TransactionAggregationJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ── 4. 소스 스트림 ──
        DataStream<PaynexEvent> txStream = env
                .fromSource(kafkaSource, wmStrategy, "PayNex Kafka Source")
                .filter(event -> event != null)
                .filter(event -> {
                    String type = event.getEventType();
        ...
        KafkaSink<String> aggSink = ExactlyOnceKafkaSinkBuilder.build(
                KAFKA_BROKERS, "paynex.aggregation.5min", "flink-agg-tx");
        ...
        KafkaSink<String> slidingSink = ExactlyOnceKafkaSinkBuilder.build(
                KAFKA_BROKERS, "paynex.aggregation.sliding", "flink-sliding-tx");
        ...
        KafkaSink<String> dlqSink = ExactlyOnceKafkaSinkBuilder.build(
                KAFKA_BROKERS, "paynex.events.dlq", "flink-dlq-tx");
        ...
        // ── 10. 실행 ──
        env.execute("PayNex Transaction Aggregation Job v3.0 (Exactly-once)");
    }
}
JAVAEOF
```
...
docker exec lab-kafka-1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server kafka-1:9092 \
  --create --topic paynex.aggregation.5min \
  --partitions 3 \
  --replication-factor 3 \
  --config min.insync.replicas=2
...
docker exec lab-kafka-1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server kafka-1:9092 \
  --create --topic paynex.aggregation.sliding \
  --partitions 3 \
  --replication-factor 3 \
  --config min.insync.replicas=2
...
echo "[3] Kafka 토픽 검증"
docker exec lab-kafka-1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server kafka-1:9092 --list 2>/dev/null | grep -q "paynex.events.ingested"
check $? "소스 토픽 paynex.events.ingested 존재"

docker exec lab-kafka-1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server kafka-1:9092 --list 2>/dev/null | grep -q "paynex.aggregation.5min"
check $? "집계 토픽 paynex.aggregation.5min 존재"

docker exec lab-kafka-1 /opt/kafka/bin/kafka-topics.sh \
  --bootstrap-server kafka-1:9092 --list 2>/dev/null | grep -q "paynex.alerts.fraud"
check $? "알림 토픽 paynex.alerts.fraud 존재"
...
echo ""
echo "[6] Redis 캐시 상태"
REDIS_KEYS=$(docker exec lab-redis redis-cli -a redis1234 KEYS "fraud:*" 2>/dev/null | wc -l)
...
| 토픽 | 용도 | RF | 파티션 |
|------|------|-----|--------|
| paynex.events.ingested | NiFi 수집 데이터 (소스) | 3 | 6 |
| paynex.aggregation.5min | Tumbling 5분 집계 결과 | 3 | 3 |
| paynex.aggregation.sliding | Sliding 이동 평균 결과 | 3 | 3 |
| paynex.alerts.fraud | 이상거래 알림 | 3 | 3 |
| paynex.events.dlq | 지연·불량 데이터 | 3 | 2 |
...
# PayNex 실시간 파이프라인 아키텍처 — Week 4 완성

## 전체 흐름

```
┌──────────────────────────────────────────────────────────────────────────┐
│                    PayNex 실시간 데이터 파이프라인                          │
│                                                                          │
│  [수집 계층 — Week 3]                                                     │
│  ┌─────────┐                                                             │
...
│  [버퍼 계층 — Week 2]                                                     │
│  ┌────────────────────────────────┐                                      │
│  │ Kafka (paynex.events.ingested) │  6 파티션, RF=3, min.ISR=2           │
│  └────────────┬───────────────────┘                                      │
│               │                                                          │
│       ┌───────┴───────┐                                                  │
│       ▼               ▼                                                  │
...
| 3 | PaynexEvent.java (표준 이벤트 POJO) | ☐ |
| 4 | AggregatedResult.java (집계 결과 모델) | ☐ |
| 5 | FraudAlert.java (이상거래 알림 모델) | ☐ |
| 6 | PaynexEventDeserializer.java (Kafka JSON 역직렬화기) | ☐ |
